...
object (self)
        ...
        val mutable write_buf = Buffer.create 1000
        ...
        method write buf off len =
                Buffer.add_substring write_buf buf off len

        method flush =
                let encoded_size = encode_frame_size (Int32.of_int (Buffer.length write_buf)) in
                transport#write encoded_size 0 (String.length encoded_size);
                transport#write (Buffer.contents write_buf) 0 (Buffer.length write_buf);
                transport#flush;
